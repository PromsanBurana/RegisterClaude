...
function CourseCard({
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -6 }}
      className={`group relative rounded-3xl border border-line bg-surface shadow-card overflow-hidden transition-shadow hover:shadow-elevated ${lift}`}
    >
      {/* Top gradient bar */}
      <div
        aria-hidden
        ...
        <div className="mt-8 pt-7 border-t border-line">
          ...
          <div className="mt-4 grid sm:grid-cols-2 gap-3">
            {course.batches.map((b) => (
              ...
              >
                <p className="text-xs font-semibold text-fg-muted">{b.label}</p>
                <p className="mt-1 text-sm font-semibold text-ink">{b.date}</p>
